...
## Web GUI — Getting Started

A full browser-based GUI is included in the `gui/` folder. It provides real-time terminal output, partition ops, sector ops, memory access, modem partition maps, and an Info reference tab — no command-line knowledge required.

### Step 1 — Get the project

```bash
git clone https://github.com/lbraskus/edl-gui
cd edl-gui
...
```bash
pip install flask flask-socketio pyusb pyserial lxml pycryptodome capstone
```
...
1. Install **Python 3.9+** from [python.org](https://www.python.org) — check *"Add Python to PATH"* during install
2. Install **UsbDk** (USB driver) from [github.com/daynix/UsbDk/releases](https://github.com/daynix/UsbDk/releases)
   - Alternative: use [Zadig](https://zadig.akeo.ie/) → install WinUSB for the device (VID 0x05C6 PID 0x9008)
3. **Run terminal as Administrator** (right-click → Run as administrator)
4. Start the GUI:
   ```cmd
   python gui\app.py
   ```
...
> **Note:** Windows Defender / antivirus may flag pyusb USB access. Add the project folder to exclusions if it blocks the connection.
...
1. Install system packages:
   ...
   ```
2. Install the udev rule so non-root users can access the device:
   ```bash
   sudo cp Drivers/51-edl.rules /etc/udev/rules.d/
   sudo udevadm control --reload-rules
   sudo udevadm trigger
   ```
3. Disable ModemManager (it hijacks the USB port):
   ```bash
...
> **Tip:** If you get a `Permission denied` USB error, either add yourself to the `plugdev` group (`sudo usermod -aG plugdev $USER` then log out/in) or run with `sudo`.
...
3. Install Python dependencies:
   ```bash
   pip3 install flask flask-socketio pyusb pyserial lxml pycryptodome capstone
   ```
4. Start the GUI:
   ```bash
   python3 gui/app.py
...
> **Note:** macOS may block USB access for unsigned apps. Go to **System Settings → Privacy & Security** and allow access if prompted.
...
| Step | Action |
|------|--------|
| 1 | Put your device in EDL mode (USB PID 0x9008) |
| 2 | Select **EDL Mode (0x05C6 / 0x9008)** from the *Device Mode* dropdown — VID/PID fill automatically |
| 3 | Click **SCAN** — detects device on USB and fills fields |
| 4 | Click **CONNECT** — sends a NOP to confirm the EDL session |
| 5 | Use the tabs to read/write/erase partitions or sectors |

### GUI Tabs

| Tab | What it does |
|-----|-------------|
| **Partition Ops** | Read, write, erase partitions by name (GPT-based) |
| **Sector Ops** | Raw LBA sector access — bypasses GPT entirely |
| **Memory** | Peek/poke physical memory addresses |
| **Device** | NOP, reset, get storage info, set bootable LUN |
| **Modem Partitions** | Visual eMMC map + pre-filled modem partition actions |
| **Advanced** | Free-form custom arguments for any edl.py command |
| **Info** | Command reference, USB ID table, protocol quick facts |

### Command-line usage (without GUI)

```bash
# Windows
python edl.py printgpt --vid=0x05C6 --pid=0x9008

# Linux / macOS
python3 edl.py printgpt --vid=0x05C6 --pid=0x9008

# Read a partition
python3 edl.py r modem modem.bin --vid=0x05C6 --pid=0x9008

# Write a partition
python3 edl.py w modem NON-HLOS.bin --vid=0x05C6 --pid=0x9008

# Dump all partitions to a folder
python3 edl.py rl dump_folder --vid=0x05C6 --pid=0x9008

# Erase a partition
python3 edl.py e userdata --vid=0x05C6 --pid=0x9008
```
